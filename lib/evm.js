...
export class Rpc {
   ...
   *
   * Two passes. One pass over eight endpoints is not enough, and the reason is measured rather
   * than theoretical: on 2026-09-25 the page announced "all 8 endpoints failed for
   * eth_getBlockByNumber" while a probe of the SAME eight endpoints, moments later, found seven
   * answering normally in under a second. A transient local condition — a burst of DNS failures,
   * a network hiccup, a machine under load — had taken down one pass, and because the whole read
   * is retried only on the next 60-second tick, a reader who loaded the page in that window saw
   * "Cannot read on-chain data. None of the configured RPC endpoints responded" over a fleet that
   * was, in fact, healthy.
   *
   * That sentence is the most damaging thing this page can print, so a single pass is too cheap
   * a trigger for it. A second, evenly spaced pass turns a one-in-N transient into a non-event
   * while still failing in bounded time (2 x 8 x timeout worst case, and the first pass usually
   * fails fast rather than timing out).
   *
   * Deliberately NOT retried: a definitive answer — a revert, or a JSON-RPC error the endpoint
   * says is the call's own fault. Those are the same everywhere and re-asking just delays the
   * honest "this one value could not be read". */
  async call(method, params = []) {
    ...
    for (let pass = 0; pass < PASSES; pass++) {
      for (let k = 0; k < this.urls.length; k++) {
        /* A retry pass starts one endpoint further along, so it does not open on the endpoint that
         * just failed. `start + pass` shifts the whole pass rather than offsetting within it: an
         * in-pass offset (start + pass + k) walks the same set but wraps at a different place,
         * which silently repeats one endpoint and skips another. */
        const idx = (start + pass + k) % this.urls.length;
        ...
        try {
          ...
          if (json.error) {
            ...
            // including across passes, hence the labelled throw rather than a `break`,
            // which would only have left the inner loop and started pass 2 anyway.
          ...
          if (err && err.definitive) throw err; // same reason: a revert ends the whole call
          lastErr = err;
          this.health[idx] = { url, ok: false, ms: Date.now() - t0, error: String(err.message || err) };
        }
      }
    }
    throw new RpcError(`all ${this.urls.length} endpoints failed for ${method} (${PASSES} passes): ${lastErr && lastErr.message}`);
  }
 ...
 *
 * Returns null for a value that is not a bigint rather than throwing a BigInt/number mix
 * error — same reasoning as fmt18() below. Callers that divide by the result already have
 * to handle a non-number, so null is the honest answer for "not read". */
...
/** Is this an 18-decimal bigint we can format? */
function isWei(v) {
 ...
 *
 * A value the chain did not give us must not take the whole page down with it, and it
 * used to: `undefined < 0n` is `false` in JavaScript — the comparison silently does NOT
 * throw — so the guard below let `undefined` through to `-v`, and `-undefined` is the
 * point where a BigInt meets a non-BigInt and the runtime raises
 * "Cannot mix BigInt and other types". One unread view (pendingTrim, inventoryCap,
 * capFloor, tokensInPool) therefore threw out of renderVerdict(), and renderAll() — which
 * calls the renderers in sequence — never reached the other fourteen. The page was left
 * half blank while every other reading was perfectly fine, and the banner blamed the RPC
 * fleet instead of the one field.
 *
 * So the type check comes FIRST, before any arithmetic, and the caller gets a string back
 * rather than an exception. The page already has a convention for this: `unavailable`,
 * translated at render time. Callers that want to distinguish "could not read" from "read
 * as zero" can test the value themselves — 0 is a claim, and this must not look like one. */
export const UNREADABLE = "unavailable";

export function fmt18(v, dp = 6, group = true) {
  if (!isWei(v)) return UNREADABLE;
  const neg = v < 0n;
  const abs = neg ? -v : v;
  const base = 10n ** 18n;
...
/** Format with an arbitrary number of decimals. Same type-first contract as fmt18().
 *
 * `decimals` accepts a Number as well as a BigInt, because callers legitimately hold both:
 * app.js passes the snapshot's `simdDecimals` (already normalised to a BigInt) while
 * scripts/selftest.mjs and the collect scripts pass a plain 18 or 24. Rejecting Number here
 * would have been a guard that broke a working call rather than a broken one — and it did:
 * selftest went red on `fmtUnits(x, 24, 2)` and, because that suite is the first link in
 * `npm test`'s chain, it stopped the other twelve suites from running at all. Only a
 * non-numeric or negative decimals is actually unrepresentable. */
export function fmtUnits(v, decimals, dp = 6, group = true) {
  if (!isWei(v)) return UNREADABLE;
  if (typeof decimals !== "bigint" && typeof decimals !== "number") return UNREADABLE;
  /* Number.isInteger first: BigInt(NaN) and BigInt(1.5) throw, and a guard that throws on the
   * value it is meant to reject is not a guard. */
  if (typeof decimals === "number" && !Number.isInteger(decimals)) return UNREADABLE;
  const dec = BigInt(decimals);
  if (dec < 0n) return UNREADABLE;
  const neg = v < 0n;
  const abs = neg ? -v : v;
  const base = 10n ** dec;
  const whole = abs / base;
  const frac = abs % base;
  let fs = frac.toString().padStart(Number(dec), "0").slice(0, dp);
  if (dp === 0) fs = "";
  let ws = whole.toString();
  if (group) ws = ws.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return (neg ? "-" : "") + ws + (fs ? "." + fs : "");
...
/** Does a value exist as a bigint? For call sites that must branch rather than print. */
export { isWei };
